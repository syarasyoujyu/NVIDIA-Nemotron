from .bit_manipulation import EXTRACTOR as BIT_EXTRACTOR
from .bit_manipulation import VALIDATOR as BIT_VALIDATOR
from .gravity import EXTRACTOR as GRAVITY_EXTRACTOR
from .gravity import VALIDATOR as GRAVITY_VALIDATOR
from .numeric_equation import DEDUCE_EXTRACTOR as NUMERIC_EQUATION_DEDUCE_EXTRACTOR
from .numeric_equation import DEDUCE_VALIDATOR as NUMERIC_EQUATION_DEDUCE_VALIDATOR
from .numeric_equation import EXTRACTOR as NUMERIC_EQUATION_EXTRACTOR
from .numeric_equation import GUESS_EXTRACTOR as NUMERIC_EQUATION_GUESS_EXTRACTOR
from .numeric_equation import GUESS_VALIDATOR as NUMERIC_EQUATION_GUESS_VALIDATOR
from .numeric_equation import VALIDATOR as NUMERIC_EQUATION_VALIDATOR
from .numeral import EXTRACTOR as ROMAN_EXTRACTOR
from .numeral import VALIDATOR as ROMAN_VALIDATOR
from .cryptarithm import DEDUCE_EXTRACTOR as SYMBOL_EQUATION_DEDUCE_EXTRACTOR
from .cryptarithm import DEDUCE_VALIDATOR as SYMBOL_EQUATION_DEDUCE_VALIDATOR
from .cryptarithm import EXTRACTOR as SYMBOL_EQUATION_EXTRACTOR
from .cryptarithm import GUESS_EXTRACTOR as SYMBOL_EQUATION_GUESS_EXTRACTOR
from .cryptarithm import GUESS_VALIDATOR as SYMBOL_EQUATION_GUESS_VALIDATOR
from .cryptarithm import VALIDATOR as SYMBOL_EQUATION_VALIDATOR
from .cipher import EXTRACTOR as TEXT_EXTRACTOR
from .cipher import VALIDATOR as TEXT_VALIDATOR
from .unit_conversion import EXTRACTOR as UNIT_EXTRACTOR
from .unit_conversion import VALIDATOR as UNIT_VALIDATOR


def get_extractors():
    return [
        BIT_EXTRACTOR,
        TEXT_EXTRACTOR,
        ROMAN_EXTRACTOR,
        UNIT_EXTRACTOR,
        GRAVITY_EXTRACTOR,
        NUMERIC_EQUATION_EXTRACTOR,
        NUMERIC_EQUATION_DEDUCE_EXTRACTOR,
        NUMERIC_EQUATION_GUESS_EXTRACTOR,
        SYMBOL_EQUATION_EXTRACTOR,
        SYMBOL_EQUATION_DEDUCE_EXTRACTOR,
        SYMBOL_EQUATION_GUESS_EXTRACTOR,
    ]


def get_validators():
    return [
        BIT_VALIDATOR,
        TEXT_VALIDATOR,
        ROMAN_VALIDATOR,
        UNIT_VALIDATOR,
        GRAVITY_VALIDATOR,
        NUMERIC_EQUATION_VALIDATOR,
        NUMERIC_EQUATION_DEDUCE_VALIDATOR,
        NUMERIC_EQUATION_GUESS_VALIDATOR,
        SYMBOL_EQUATION_VALIDATOR,
        SYMBOL_EQUATION_DEDUCE_VALIDATOR,
        SYMBOL_EQUATION_GUESS_VALIDATOR,
    ]
