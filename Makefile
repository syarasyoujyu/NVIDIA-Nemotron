...
RAW_DATA_ARGS ?= --mode $(RAW_DATA_MODE) --seed $(RAW_DATA_SEED) --output-dir $(RAW_PATTERN_DIR) --train-counts "$(RAW_TRAIN_COUNT_LIST)" --test-counts "$(RAW_TEST_COUNT_LIST)" $(if $(RAW_FAIL_ON_COT_MISMATCH),--fail-on-cot-mismatch,)
# task type order:
